...
class RAGService:
    ...
    def retrieve_context(self, query: str, top_k: int = 2) -> List[Dict[str, Any]]:
        ...
        if query_res and query_res["documents"] and len(query_res["documents"][0]) > 0:
            for i in range(len(query_res["documents"][0])):
                ...
                # Filter out weak cosine distance matches
                if distance < 0.90:
                    retrieved.append({
    ...
    def generate_answer(self, query: str) -> Tuple[str, List[Dict[str, str]]]:
        ...
        # 1. Friendly conversational greetings
        if clean_query in ["hi", "hello", "hey", "good morning", "good afternoon"]:
            return (
                "Hello! How can I help you today? Feel free to ask about our remote working policy, VPN setup, password reset, or expense claims.",
                []
            )

        sources = self.retrieve_context(query, top_k=2)

        # 2. Vector distance guardrail (no matching docs)
        if not sources:
...
'{FALLBACK_MESSAGE}'
3. Keep the tone clear, warm, and complete. Do not extrapolate or invent policies.
...
ANSWER:"""
        ...
        candidate_models = list(dict.fromkeys([self.model_name, "gemini-2.5-flash", "gemini-3.1-flash-lite", "gemini-3.7-flash"]))
            ...
            for model in candidate_models:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # 3. Model output guardrail check
                    ...
                    logger.warning(f"Model {model} returned status {response.status_code}: {last_error}")
